...
def train_dqn_models(task='ReadySetGo-v0', dt=20, num_episodes=3000, gamma=0.99,
                     epsilon_start=1.0, epsilon_end=0.1, epsilon_decay=1500,
    ...
    # ReadySetGo targets can be 40-80 timesteps (800-1600ms) with dt=20
    # Need enough steps to reach target + some buffer
    max_steps_per_episode = 200  # Increased to allow reaching all targets
    ...
    for ep in range(1, num_episodes + 1):
        ...
        for key in policy_nets.keys():
            ...
            steps = 0
            has_pressed_go = False  # Track if go was already pressed
            while not done and steps < max_steps_per_episode:
                ...
                if target_go_ms is not None:
                    target_go = int(target_go_ms / env.dt)  # Convert ms to timesteps
                    if action == go_idx:
                        if has_pressed_go:
                            # Already pressed go once - moderate penalty to reduce spam
                            shaped_reward = -0.1 # Reduced from -2.0 to be less harsh during exploration
                        else:
                            has_pressed_go = True
                            timing_error = abs(steps - target_go)
                            # Wider window initially: within 50 steps = 1000ms
                            if timing_error <= 50:
                                timing_bonus = max(0.0, 15.0 - 0.2 * timing_error)  # Higher max reward
                                shaped_reward = timing_bonus  # 0 to +15.0 based on accuracy
                            else:
                                # Pressed go too early/late - small penalty
                                shaped_reward = -0.25  # Reduced from -0.5
                    else:
                        # Action is fixate (0)
                        shaped_reward = -0.005  # Small penalty for fixating - forces agent to act
                else:   
