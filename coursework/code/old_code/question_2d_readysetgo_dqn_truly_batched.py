"""
TRULY BATCHED DQN - Forward passes are batched together for real speedup
Key insight: Process all parallel environments in a single forward pass
"""
import os
import random
...
def pad_sequences_batch(sequences, max_len):
    """
    Pad list of sequences to same length
    Args:
        sequences: list of tensors [seq_len, 1, obs_dim]
        max_len: target length
    Returns:
        stacked tensor [batch, max_len, obs_dim]
    """
    batch = []
    for seq in sequences:
        ...
        if seq_len < max_len:
            ...
            seq = seq[-max_len:]
        batch.append(seq)
    return torch.stack(batch)  # [batch, max_len, obs_dim]


def select_action_truly_batched(policy_net, obs_sequences, epsilon, action_space, device, num_parallel):
    """
    TRULY BATCHED: Process all environments in ONE forward pass

    Note: We sacrifice hidden state persistence across steps for massive speedup.
    This is acceptable because we're feeding sequences of observations anyway.
    """
    actions = []

    if random.random() < epsilon:
        # Exploration: random actions (no forward pass needed)
        return [action_space.sample() for _ in range(num_parallel)]

    # Exploitation: Batch all sequences together
    with torch.no_grad():
        # Stack all sequences: list of [seq_len, 1, obs_dim] -> [seq_len, num_parallel, obs_dim]
        # First pad them to same length
        obs_batch = pad_sequences_batch(obs_sequences, obs_sequences[0].shape[0])  # [num_parallel, seq_len, obs_dim]
        obs_batch = obs_batch.permute(1, 0, 2)  # [seq_len, num_parallel, obs_dim]
        obs_batch = obs_batch.to(device)

        # ONE forward pass for all parallel envs!
        q_vals, _, _ = policy_net(obs_batch)  # [seq_len, num_parallel, act_dim]
        q_vals_last = q_vals[-1]  # [num_parallel, act_dim]

        # Get actions for all envs
        actions = q_vals_last.argmax(dim=1).cpu().numpy().tolist()
...
def optimize_model(policy_net, target_net, buffer, optimizer, batch_size, gamma, device, model_type, max_seq_len=20):
    ...
    state_batch = pad_sequences_batch(batch.obs_sequence, max_seq_len).to(device)
    action_batch = torch.tensor(batch.action, dtype=torch.long, device=device).unsqueeze(1)
    reward_batch = torch.tensor(batch.reward, dtype=torch.float32, device=device).unsqueeze(1)
    next_state_batch = pad_sequences_batch(batch.next_obs_sequence, max_seq_len).to(device)
...
def plot_training_all(histories, params=None, output_path='images/question_2d_readysetgo_dqn_rewards.png'):
    ...
    ax.set_title('ReadySetGo-v0 DQN (TRULY BATCHED): Reward (smoothed)', fontsize=15, fontweight='bold')
    ax.legend(fontsize=11)
    ax.tick_params(axis='both', labelsize=11)
    ax.grid(True, alpha=0.3)
    if params:
        text = (f"episodes={params.get('num_episodes')}, num_parallel={params.get('num_parallel')}, "
                f"lr={params.get('lr')}, gamma={params.get('gamma')}, batch={params.get('batch_size')}")
...
def train_dqn_models(task='ReadySetGo-v0', dt=20, num_episodes=2000, gamma=0.99,
                     ...
                     hidden_size=64, device='cpu', max_seq_len=20, num_parallel=16):
    """
    TRULY BATCHED VERSION: All parallel envs processed in single forward pass
    Trade-off: No hidden state persistence for ~16x speedup
    """
    env_kwargs = {'dt': dt}
    ...
    for key, mtype in model_types:
        net = Net(obs_dim, hidden_size, act_dim, model_type=mtype, dt=dt, tau=150, sigma_rec=0.1,
                  exc_ratio=0.8 if mtype == 'bio_realistic' else None).to(device)
    ...
    print(f"Starting TRULY BATCHED training for {num_episodes} episodes...")
    print(f"num_parallel = {num_parallel} (TRUE batching: 1 forward pass for all envs)")
    print(f"max_seq_len = {max_seq_len}")
    print(f"Expected speedup: ~{num_parallel}x vs sequential")
    print()
    ...
    for ep in range(1, num_episodes + 1):
        ...
        for model_idx, (key, mtype) in enumerate(model_types):
            ...
            while not all(done_list) and max(steps_list) < max_steps_per_episode:
                ...
                # TRULY BATCHED action selection (ONE forward pass!)
                actions = select_action_truly_batched(
                    policy_nets[key], obs_sequences, epsilon, envs[0].action_space, device, num_parallel
                )
                ...
                for i in range(num_parallel):
                    ...
                    # CRITICAL FIX: Only optimize every 4 steps (not every step!)
                    if global_steps[key] % 4 == 0:
    ...
    }, output_path='images/question_2d_readysetgo_dqn_truly_batched_rewards.png')
    ...
    torch.save({
        ...
        'task': task,
    }, 'checkpoints/question_2d_readysetgo_dqn_truly_batched.pt')
    print("Saved checkpoint: checkpoints/question_2d_readysetgo_dqn_truly_batched.pt")
...
if __name__ == '__main__':
    ...
    # Truly batched: 16 envs processed in 1 forward pass!
