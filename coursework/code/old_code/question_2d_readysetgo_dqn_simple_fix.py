...
def select_action(policy_net, obs_sequence, epsilon, action_space, device):
    """
    CRITICAL FIX 1: Use observation sequence instead of single timestep
    Note: We don't persist hidden state externally - it's handled internally by the RNN
    """
    if random.random() < epsilon:
        return action_space.sample()
    with torch.no_grad():
        # obs_sequence shape: [seq_len, 1, obs_dim]
        q_vals, _ = policy_net(obs_sequence.to(device))
...
def optimize_model(policy_net, target_net, buffer, optimizer, batch_size, gamma, device, model_type, max_seq_len=50):
    ...
    q_values, rnn_activity = policy_net(state_batch)
...
def train_dqn_models(task='ReadySetGo-v0', dt=20, num_episodes=3000, gamma=0.99,
                     ...
                     hidden_size=64, device='cpu', max_seq_len=30):
    """
    CRITICAL FIXES IMPLEMENTED:
    1. Use observation sequences (max_seq_len) instead of single timesteps
    2. (Hidden state persistence is internal to RNN - handled automatically)
    ...
    # CRITICAL FIX: Reduce max steps - intervals are 500-2000ms with gain 1.5 = max ~3000ms = 150 steps @ 20ms
    ...
    print(f"  2. Hidden states managed internally by RNN")
    ...
    for ep in range(1, num_episodes + 1):
        ...
        ep_steps = {k: 0 for k in policy_nets.keys()}
        ...
        for key, mtype in model_types:
            ...
            # CRITICAL FIX 1: Maintain observation history
            ...
            while not done and steps < max_steps_per_episode:
                ...
                # Select action
                action = select_action(policy_nets[key], obs_sequence, epsilon, env.action_space, device)
                ...
                if target_go_ms is not None:
                    ...
                    if action == go_idx:
                        if has_pressed_go:
                            ...
                            has_pressed_go = True
                            # Use environment's reward structure
                        ...
                        # CRITICAL FIX 3: NO penalty for fixating
                        shaped_reward = reward
                else:
                    shaped_reward = reward
        ...
        if ep == 1 or ep % 50 == 0:
            msg = " | ".join([f"{k}: R={ep_rewards[k]:.2f}" for k in policy_nets.keys()])
