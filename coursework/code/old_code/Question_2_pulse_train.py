...
def evaluate_model_balanced(net, env, num_trials=500):
    """Evaluate model on PulseDecisionMaking task with balanced metrics."""
    net.eval()
...
if __name__ == '__main__':
    ...
    print("=" * 70)
    print("Question 2: Training Models on PulseDecisionMaking-v0")
    print("=" * 70)
    print(f"\nTraining: Vanilla={TRAIN_VANILLA}, Leaky={TRAIN_LEAKY}, "
          f"Leaky+FA={TRAIN_LEAKY_FA}, Bio={TRAIN_BIO}\n")

    print("[1] Setting up NeuroGym PulseDecisionMaking-v0 task...")
    task = 'PulseDecisionMaking-v0'
    kwargs_env = {'dt': 100}
    seq_len = 100
    ...
    print(f"Task: {task}")
    print(f"Description: Count pulses in two channels, choose channel with more pulses")
    ...
    torch.save(save_payload, 'checkpoints/question_2_pulse_models_and_data.pt')
    print("Saved: checkpoints/question_2_pulse_models_and_data.pt")
    print()
    ...
    print("  - Decision-making task: Count pulses, choose channel with more")
    print("  - Tests evidence accumulation and integration")
    print("  - 3 input features (fixation + 2 pulse channels)")
    print("  - 3 output actions (fixate, choose left, choose right)")
    print("  - ~22 timesteps per trial")
    print("\nExpected Performance:")
    print("  - This is an EASY task - models should achieve >90% accuracy")
    print("  - Tests ability to integrate discrete events over time")
    print("  - All architectures should perform well")
    print("\nNext steps:")
    print("  Run Question_2_pulse_analysis.py to generate visualizations")
    print("=" * 70)
